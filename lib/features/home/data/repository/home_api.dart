...
class HomeApi {
  ...
  Future getAutocompletePlaceLatLng({
    ...
    // try {
    //   pl.FlutterGooglePlacesSdk place =
    //       pl.FlutterGooglePlacesSdk(AppConstants.mapKey);
    //   pl.FetchPlaceResponse value =
    //       await place.fetchPlace(placeId, fields: [pl.PlaceField.Location]);
    //   return value.place!.latLng;
    // } catch (e) {
    //   debugPrint(e.toString());
    //   rethrow;
    // }
    try {
      ...
      dynamic latLng;
      Response response = await DioProviderImpl().get(
        // 'https://maps.googleapis.com/maps/api/place/details/json?placeid=$placeId&key=${AppConstants.mapKey}',
      ...
      //  latLng= LatLng(response.data['result']['geometry']['location']['lat'],
